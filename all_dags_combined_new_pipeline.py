...
# trigger_export >> set_current_space_id >> create_global_folder_task >> nodejs_space_export_to_plxml >> get_plmxml_structure_file >> [download_files] >> list_jt_files_task >> end_of_dag
...
# Loop over JT files and trigger STEP conversion for each file
def trigger_step_convert(jt_files, **kwargs):
    for i, jt_file in enumerate(jt_files):
        task_id = f'trigger_step_convert_{i}'
        bash_command = (
            'cd /opt/airflow/tempSRCfiles/coretech-2024-linux/build && '
        ...
        # Set dependencies
        kwargs['ti'].xcom_push(key=f'convert_task_{i}', value=task_id)
        kwargs['ti'].xcom_push(key=f'convert_command_{i}', value=bash_command)
...
# Set task dependencies without creating a cycle
trigger_export >> set_current_space_id >> create_global_folder_task >> nodejs_space_export_to_plxml >> get_plmxml_structure_file >> download_files >> list_jt_files_task >> trigger_step_convert_task >> end_of_dag
